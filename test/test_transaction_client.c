...
int main()
{
	...
	transaction_client* tcli = new_transaction_client(cgp, 3);
	...
	{
		inpt[i] = get_literal_cstring(inpt_strs[i]);
...
void* transaction(int fd, int* close_connection_requested, void* additional_params)
{
	...
	int buffsentlength = send(fd, get_byte_array_dstring(data_to_send), get_char_count_dstring(data_to_send), 0);
